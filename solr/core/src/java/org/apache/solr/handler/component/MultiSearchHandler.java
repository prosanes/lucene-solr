...
public class MultiSearchHandler extends SearchHandler {
  ...
  private void splitComponents() {
    ...
    queryComponent.add(getComponents().get(0));
  }
  ...
  private void renameLastResponse(SolrQueryResponse rsp, int request_number) {
    NamedList<Object> values = rsp.getValues();
    int indexOfResponse = values.indexOf("response", 0);
    values.setName(indexOfResponse, (request_number+1)+".response");
  }
